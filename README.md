...
Ниже — полный путь от нуля. Команды для **macOS / Linux** (`curl`) и **Windows** (`curl.exe`).

### Оценка продукта (публичный клон, без ядра)
...
http://localhost:8080 — оболочка. Paper и плейбуки не запускаются.

### Оператор (ядро `../IMOEX-core`)

Нужны Java 17+, Maven 3.9+, соседняя папка `IMOEX-core`, `application-local.yml` с `imoex.run.unlock`.
...
Профиль `operator` включается сам, если существует `../IMOEX-core/pom.xml`.
...
Клон / каталог проекта (**корень репозитория IMOEX**, не подпапка `moex/`):
...
├── trinity-calendar-arb/        # strategy 3 — FORTS calendar + fly (T-Invest quotes, paper)
...
Код и материалы репозитория — **проприетарные**, все права принадлежат **Ивану Тюлькину (самозанятый)**.
Программный комплекс **TRINITY** имеет государственную регистрацию программы для ЭВМ в **Роспатенте**.
См. файл [`LICENSE`](LICENSE): использование, копирование, модификация, форки, клонирование, распространение
и коммерческое (и иное) применение **без письменного согласия запрещены**. Разрешено — ничего.

Рыночные данные принадлежат **Московской бирже** (публичный ISS API и его условия).
