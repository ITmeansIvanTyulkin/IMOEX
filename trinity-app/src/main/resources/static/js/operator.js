(function () {
  ...
  function bindStatementPagers() {
    const wraps = document.querySelectorAll(".statement-table-pager");
    ...
    wraps.forEach(function (wrap) {
      ...
      const pageSize = Math.max(1, parseInt(wrap.getAttribute("data-page-size") || "20", 10) || 20);
      if (rows.length <= pageSize) return;

      let page = 1;
      const pages = Math.ceil(rows.length / pageSize);
      let nav = wrap.querySelector(".statement-pager");
      if (!nav) {
        nav = document.createElement("nav");
        nav.className = "statement-pager";
        nav.setAttribute("aria-label", "Страницы таблицы сделок");
      ...
      function showPage(p) {
        ...
        rows.forEach(function (tr, i) {
          tr.hidden = i < from || i >= to;
        });
        const bits = [];
        bits.push('<span class="statement-pager-meta">Сделки '
          + (from + 1) + "–" + Math.min(to, rows.length)
          + " из " + rows.length + "</span>");
        bits.push('<div class="statement-pager-pages">');
        for (let i = 1; i <= pages; i++) {
          bits.push('<button type="button" class="statement-pager-btn'
            + (i === page ? " is-active" : "")
            + '" data-page="' + i + '" aria-label="Страница ' + i + '"'
      ...
      wrap.__statementShowPage = showPage;

      if (!wrap.dataset.pagerBound) {
        wrap.dataset.pagerBound = "1";
        nav.addEventListener("click", function (ev) {
          const btn = ev.target && ev.target.closest
            ? ev.target.closest(".statement-pager-btn")
            : null;
          if (!btn) return;
          ev.preventDefault();
  ...
  function bindStatementPdfExport() {
    const btn = $("statement-export-pdf");
    if (!btn) return;
    btn.addEventListener("click", function () {
      const prev = btn.textContent;
      btn.disabled = true;
      btn.textContent = "Сохраняю…";
      fetch("/api/statement/export.pdf", { credentials: "same-origin" })
        .then(function (res) {
          ...
          var name = star
            ...
            : (plain ? plain[1] : "TRINITY-statement.pdf");
          if (name.indexOf("=?") === 0) name = "TRINITY-statement.pdf";
        ...
        .finally(function () {
          btn.disabled = false;
          btn.textContent = prev || "Сохранить PDF";
        });
    });
  }
  ...
  function saveTourState(s) {
    try { localStorage.setItem(TOUR_KEY, JSON.stringify(s)); } catch (_) {}
  }
  function ensureTourUi() {
    let box = document.getElementById("trinity-tour-box");
    if (box) return box;
    box = document.createElement("div");
    box.id = "trinity-tour-box";
    box.className = "trinity-tour-box";
    box.hidden = true;
    box.innerHTML = '<div class="trinity-tour-card">'
      + '<p class="trinity-tour-kicker">Обучение TRINITY</p>'
      + '<h3 id="trinity-tour-title"></h3>'
      + '<p id="trinity-tour-body"></p>'
      + '<div class="trinity-tour-actions">'
      + '<button type="button" class="btn btn-ghost" id="trinity-tour-skip">Пропустить</button>'
      + '<button type="button" class="btn" id="trinity-tour-next">Далее</button>'
      + '</div></div>';
    document.body.appendChild(box);
    document.getElementById("trinity-tour-skip").addEventListener("click", function () {
      saveTourState({ done: true, step: TOUR_STEPS.length, at: Date.now() });
      box.hidden = true;
    });
    document.getElementById("trinity-tour-next").addEventListener("click", function () {
      const st = tourState();
      const next = (st.step || 0) + 1;
      if (next >= TOUR_STEPS.length) {
        saveTourState({ done: true, step: next, at: Date.now() });
        box.hidden = true;
        return;
      }
      saveTourState({ done: false, step: next, at: Date.now() });
      const step = TOUR_STEPS[next];
      if (step.path && location.pathname.replace(/\/+$/, "") !== step.path.replace(/\/+$/, "")) {
        location.href = step.path + "?tour=1";
        return;
      }
      showTourStep(next);
    });
  ...
  function showTourStep(i) {
    ...
    if (!step) return;
    const box = ensureTourUi();
    document.getElementById("trinity-tour-title").textContent = (i + 1) + "/" + TOUR_STEPS.length + " · " + step.title;
    document.getElementById("trinity-tour-body").textContent = step.body;
    box.hidden = false;
  }
  function startTour(force) {
    const st = tourState();
    if (!force && st.done) return;
    const step = force ? 0 : (st.step || 0);
    saveTourState({ done: false, step: step, at: Date.now() });
    const cur = TOUR_STEPS[step];
    if (cur && cur.path && location.pathname.replace(/\/+$/, "") !== cur.path.replace(/\/+$/, "")) {
      location.href = cur.path + "?tour=1";
      return;
    }
    showTourStep(step);
  }
  document.addEventListener("click", function (e) {
    const t = e.target && e.target.closest && e.target.closest("[data-tour-start], #trinity-tour-start");
    if (t) {
      e.preventDefault();
      startTour(true);
    }
  });
  if (/\btour=1\b/.test(location.search || "")) {
    setTimeout(function () { startTour(false); }, 400);
  } else {
    const st = tourState();
    if (!st.done && !st.dismissed && location.pathname.replace(/\/+$/, "") === "/view") {
      // first visit hint only on dashboard — do not auto-force
    }
  }
