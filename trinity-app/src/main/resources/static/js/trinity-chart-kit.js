...
(function (global) {
  ...
  function footprintSkew(fp) {
    ...
    return { buy: buy, sell: sell, tot: tot, delta: buy - sell, levels: fp.levels };
  }

  function hashPick(key, variants) {
    if (!variants || !variants.length) return "";
    const s = String(key == null ? "x" : key);
    let h = 2166136261;
  ...
  function whaleHint(fp, volMult, dump, pts, seed) {
    const hugeVol = volMult >= 2.8;
    const oneSided = !!(fp && ((dump && fp.sell > fp.buy * 1.45) || (!dump && fp.buy > fp.sell * 1.45)));
    let concentrated = false;
    if (fp && fp.levels && fp.levels.length >= 3) {
      const sizes = [];
      fp.levels.forEach(function (l) {
        const v = (Number(l.buy) || 0) + (Number(l.sell) || 0);
        if (v > 0) sizes.push(v);
      });
      if (sizes.length >= 3) {
        let sum = 0;
        ...
        concentrated = max >= (sum / sizes.length) * 3.2;
      }
    }
    const whale = (hugeVol && oneSided) || (hugeVol && concentrated) || (volMult >= 4 && pts >= 28);
    if (!whale) return { text: "", whale: false };
    const dumpTexts = [
      "Похоже, здесь продавал крупный игрок: объём в разы выше обычного, лента почти вся в продажи.",
      "Так не торгует розница — слишком жирный объём в одну сторону. На рынке это называют «китом»: пачка крупных заявок, не тики по одному лоту.",
      "Ударный объём вниз: кто-то выгрузил размер. Не новость из телеги — след крупной продажи в ленте."
    ];
    const rallyTexts = [
      "Похоже, здесь покупал крупный игрок: объём аномальный, лента почти вся в покупки.",
      "Жирный объём вверх — так заходят «киты» или пачка крупных заявок. Розница так стакан не съедает.",
      "На одном ходе слишком много лотов: рынок толкнули размером, не случайным тиком."
    ];
    const mixTexts = [
      "Объём этой свечи не похож на обычную: либо крупный игрок, либо сразу несколько больших заявок.",
      "Слишком много лотов для «тихой» свечи — здесь кто-то всерьёз поучаствовал в сделке."
    ];
    const pool = oneSided ? (dump ? dumpTexts : rallyTexts) : mixTexts;
    return { text: " " + hashPick(seed + ":whale", pool), whale: true };
  }

  function impulseLesson(pattern, dump, extra) {
    extra = extra || {};
    const vol = extra.volMult;
    const pts = extra.pts || 0;
    const seed = (extra.time || "") + ":" + pattern + ":" + Math.round(pts) + ":" + (vol ? vol.toFixed(1) : "0");
    let volBit = "";
    if (vol >= 3) {
      volBit = " " + hashPick(seed + ":vol", [
        "Объём примерно ×" + vol.toFixed(1) + " к обычной свече — здесь реально торговали, не пустой скачок котировки.",
        "Лотов в разы больше, чем на соседних свечах: ход подкреплён сделками.",
        "Стакан не «пролетели по воздуху» — объём жирный, кто-то встречал цену."
      ]);
    } else if (vol > 0 && vol < 1.35) {
      volBit = " " + hashPick(seed + ":thin", [
        "Объём почти как у обычной свечи, а ход большой — между ценами почти никого не было, цена проскочила пустоту. Это не «все знали новость».",
        "Ход широкий, а сделок мало: тонкий стакан, пустые уровни. Так бывает, когда заявки сняли заранее.",
        "Пробежали много пунктов почти без объёма — воздух в стакане, не толпа."
      ]);
    } else if (vol >= 1.35) {
      volBit = " " + hashPick(seed + ":okvol", [
        "Объём выше среднего — ход подкреплён сделками.",
        "Здесь торговали активнее обычного: не один случайный тик."
      ]);
    }
    let tapeBit = "";
    const fp = extra.fp;
    if (fp) {
      if (fp.sell > fp.buy * 1.25) {
        tapeBit = " " + hashPick(seed + ":tapeS", [
          "В ленте этой свечи больше продаж: продавали сразу «по любой цене», и очередь заявок на покупку съедалась уровень за уровнем.",
          "Агрессор — продавцы: рыночными заявками снимали биды, цена шла к следующей более низкой.",
          "Покупок в ленте мало, продаж много — давление шло сверху вниз."
        ]);
      } else if (fp.buy > fp.sell * 1.25) {
        tapeBit = " " + hashPick(seed + ":tapeB", [
          "В ленте этой свечи больше покупок: покупали сразу «по любой цене», и очередь заявок на продажу съедалась уровень за уровнем.",
          "Агрессор — покупатели: рыночными заявками снимали оферы, цена шла выше.",
          "Продаж в ленте мало, покупок много — давление шло снизу вверх."
        ]);
      } else {
        tapeBit = " " + hashPick(seed + ":tapeM", [
          "В ленте покупки и продажи близки — скорее проскок пустых цен, чем одна сплошная толпа.",
          "Дельта почти в ноль: не «все продавали» и не «все покупали», а дёрнули туда-сюда."
        ]);
      }
    }
    const whale = whaleHint(fp, vol, dump, pts, seed);
    ...
    const sessionBit = extra.sessionOpen
      ? (positional
        ...
        : " " + hashPick(seed + ":open", [
          ...
          "Открытие: тонкий стакан, ночной ход выгрузили в пять минуток. Не путать с новостью.",
          "Утро сессии часто «выплёвывает» ночь — широкий бар без отдельного сюжета."
        ]))
      : "";
    const tail = volBit + tapeBit + whale.text + sessionBit;
    let why;
    let wait;
    if (pattern === "STOP_HUNT" && extra.sweptLow) {
      why = hashPick(seed + ":why", [
        "Сначала вынесли стопы под минимумом (продали туда, где почти не было покупателей), затем свеча закрылась выше — вынос не удержали.",
        "Классический «сбор лоев»: дёрнули ниже очевидный уровень, сняли стопы продавцов, и цену сразу вернули.",
        "Фитиль вниз пробил пол, но закрытие обратно выше — вынос не закрепили. Так часто охотятся за стопами, а не начинают новый тренд.",
        "Продали в пустоту под уровнем, потом откупили. Похоже на ложный пробой низа, а не на обвал."
      ]) + tail;
      wait = positional
        ? "Дёрнули вниз, часто чтобы потом идти вверх. Не шортить вынос. Позиционная не покупает «дно»: сторона только с тренда часа, вход — в промежуточную полку объёма."
        : hashPick(seed + ":wait", [
          "Типично: дёрнули вниз, чтобы потом идти вверх. Не шортить вынос. Смотрим, удержит ли цена уровень над вынесенным лоем. Робот покупает только от нижней полки дня после закрытой свечи-отбоя — не в середине выноса.",
          "Не продавать «дно выноса». Если цена останется над лоем — это был сбор стопов. Покупка у робота — только от нижней зоны дня, и только когда свеча уже закрылась отбоем.",
          "Ждём: оставят ли вынесенный лой внизу. Ловить нож в фитиле не надо. Робот не покупает середину такой свечи."
        ]);
    } else if (pattern === "STOP_HUNT") {
      why = hashPick(seed + ":why", [
        "Сначала вынесли стопы над максимумом (купили туда, где почти не было продавцов), затем свеча закрылась ниже — вынос хая не удержали.",
        "Классический «сбор хаёв»: дёрнули выше очевидный уровень, сняли стопы покупателей, и цену сразу вернули.",
        "Фитиль вверх пробил потолок, но закрытие ниже — вынос не закрепили. Так часто охотятся за стопами перед движением вниз.",
        "Купили в пустоту над уровнем, потом продали обратно. Похоже на ложный пробой верха, а не на прорыв."
      ]) + tail;
      wait = positional
        ? "Дёрнули вверх, часто чтобы потом идти вниз. Не ловить лонг на шипе. Шорт только если час вниз и цена в промежуточной полке."
        : hashPick(seed + ":wait", [
          "Типично: дёрнули вверх, чтобы потом идти вниз. Не ловить лонг на шипе. Ждём, останется ли цена под вынесенным хаем. Робот шортит от верхней полки дня только после закрытого отбоя.",
          "Не покупать верхушку выноса. Если хай оставят сверху — это был сбор стопов. Продажа у робота — только от верхней зоны дня и только по закрытой свече-отбою.",
          "Ждём: не вернут ли цену под вынесенный хай. Середину шипа робот не торгует."
        ]);
    } else if (pattern === "KNIFE") {
      why = hashPick(seed + ":why", [
        "Нож: продавали сразу по рынку. Заявки на покупку на каждом уровне исполнялись и исчезали — цена шла к следующей, более низкой. Закрытие у минимума: в этом баре покупатели так и не остановили падение.",
        "Это нож, не отбой: длинное красное тело и закрытие у самого низа. Покупатели не успели, продажи шли каскадом.",
        "Обвал внутри одной свечи — цена падала, не отскакивая. Так выглядит паника или выгрузка размера, не «спокойный» день.",
        "Закрылись у минимума: кто хотел купить, в этом баре не остановил ход. Нож режет насквозь, пока не встретит плотный объём.",
        "Продажи съедали стакан уровень за уровнем. Это не фитиль-обманка — тело говорит: давление было до закрытия."
      ]) + tail;
      wait = positional
        ? "Не ловить нож. Ждём остановку у полки объёма. Лонг только если час вверх и вход в промежуточную полку — не «дно»."
        : hashPick(seed + ":wait", [
          "Не ловить нож. Ждём остановку: сужение следующих свечей или касание нижней полки дня. Покупка у робота — только от нижней зоны после отбоя, не «догонять дно».",
          "Падающий нож не ловят руками. Смотрим, где ход затормозит. Робот не покупает середину слива.",
          "Ждём, пока свечи станут короче или цена ткнётся в нижнюю полку. Пока нож летит — это не вход."
        ]);
    } else if (pattern === "ROCKET") {
      why = hashPick(seed + ":why", [
        "Импульс вверх: покупали сразу по рынку. Заявки на продажу на каждом уровне исполнялись и исчезали — цена шла к следующей, более высокой. Закрытие у максимума: продавцы ход не остановили.",
        "Ракета, не отбой: длинное зелёное тело и закрытие у самого верха. Продавцы в этом баре не справились.",
        "Вынос вверх внутри одной свечи. Так выглядит агрессивный набор лонгов или короткий covering, не тихий набор.",
        "Закрылись у максимума: оферы снимали до последнего тика. Это импульс, пока не встретит плотный объём сверху.",
        "Покупки съедали стакан вверх. Тело, не фитиль: давление было до закрытия."
      ]) + tail;
      wait = positional
        ? "Не догонять импульс. Шорт только если час вниз и зона входа — средняя полка объёма."
        : hashPick(seed + ":wait", [
          "Не догонять вверх. Шорт у робота — только от верхней полки дня после закрытого отбоя. Если зоны нет — ждём, не остановится ли ход на хае дня.",
          "Поезд уже ушёл: середину ракеты не шортим и не докупаем. Смотрим, где ход упрётся.",
          "Ждём реакцию у верхней зоны дня. Пока свеча закрывается у хая — это не продажа."
        ]);
    } else if (pattern === "SPIKE") {
      why = hashPick(seed + ":why", [
        "Длинный фитиль и маленькое тело: цена пробежала пустые уровни и вернулась. Агрессия не закрепилась.",
        "Шип: вынесли и сразу отдали. Тело крошечное — закрепления не было, только вспышка.",
        "Дёрнули в пустоту и вернули. Так выглядит снятие стопов или тонкий стакан, не новый тренд.",
        "Фитиль длинный, тело короткое: рынок «пощупал» край и отказался там жить."
      ]) + tail;
      wait = positional
        ? "Шип сам по себе не вход. Ждём 1–2 закрытия. Позиционная торгует полку по тренду часа, не середину шипа."
        : hashPick(seed + ":wait", [
          ...
          "Не торговать сам фитиль. Смотрим следующие закрытия: оставят шип снаружи или заберут.",
          "Ждём реакцию. Шип — подсказка, где охотились за стопами, а не сигнал «входить прямо сейчас»."
        ]);
    } else {
      why = hashPick(seed + ":why", [
        dump ? "Резкий ход вниз — шире обычных свечей." : "Резкий ход вверх — шире обычных свечей.",
        dump ? "Нетипично широкая медвежья свеча: рынок ускорился вниз." : "Нетипично широкая бычья свеча: рынок ускорился вверх.",
        dump ? "Импульс вниз без «идеального» ножа: ход большой, картина смешанная." : "Импульс вверх без «идеальной» ракеты: ход большой, картина смешанная."
      ]) + tail;
      wait = positional
        ? "Середину импульса не торгуем. Смотрим, где ход остановится относительно полок объёма и тренда на часе."
        : hashPick(seed + ":wait", [
          "Не торговать середину импульса. Смотрим, где остановится относительно полок дня — и ждём реакцию, не прогноз заголовка.",
          "Сначала дать ходу выдохнуться. Вход у робота — от зоны, не из середины бара.",
          "Ждём, где импульс упрётся. Середина такого хода — самое неудобное место для входа."
        ]);
    }
    return {
  ...
  function bindCandleOhlcTip(chart, series, hostEl, opts) {
    ...
    function layoutTip(bar, x, y) {
      ...
      tip.classList.toggle("is-whale", !!(note && note.whale));
      let html = "";
      if (showFloatOhlc) {
        html += ""
      ...
      if (hasNote) {
        if (note.whale) {
          html += '<span class="trinity-ohlc-whale">Крупный объём</span>';
        }
        html += '<span class="trinity-ohlc-note">' + (note.why || note.hover) + "</span>";
        if (note.wait) {
          html += '<span class="trinity-ohlc-wait">Что дальше: ' + note.wait + "</span>";
        }
      }
      tip.innerHTML = html;
      tip.hidden = false;
